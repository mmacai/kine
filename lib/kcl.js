...
module.exports = function(config, kinesis) {
  ...
  function shards() {
    ...
    function needMore() {
      if (stop) return;
      var filteredShardList = shardList.filter(function(shard) {
        ...
        // https://docs.aws.amazon.com/AWSJavaSDK/latest/javadoc/com/amazonaws/services/kinesis/model/SequenceNumberRange.html#getEndingSequenceNumber()
        if (!shard.SequenceNumberRange.EndingSequenceNumber) {
          return true;
        }
      });
      console.log(filteredShardList);
      console.log('Number of instances', instanceList.length);
      howMany = Math.ceil(filteredShardList.length / instanceList.length);
      console.log('HOW MANY shards I need to take', howMany);
      ...
      if (Object.keys(instanceShardList).length > howMany) {
        var key = Object.keys(instanceShardList)[0];
        logger.info('Instance has too many shards, releasing' + instanceShardList[key]);
        db.releaseShard(instanceShardList[key], function (err) {
          if (err) throw err;
          delete instanceShardList[key];
          logger.info('Shard released from instance.');
        });
      }
    }
  }
  ...
  function heartbeat() {
    ...
    Object.keys(instanceShardList).forEach(function(s) {
      ...
      // Remove closed shards from instance shards
      if (instanceShardList[s].status === 'complete') {
        logger.info('Shard is in complete state, removing from instance.');
        delete instanceShardList[s];
      }
    });
